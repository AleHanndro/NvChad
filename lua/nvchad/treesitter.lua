...
--- @return table<string, boolean> List of available parsers
...
local function attach(buf, language)
  ...
  if not buffer_still_matches(buf, language) then
    return false
  end
...
function M.setup(opts)
  ...
  vim.api.nvim_create_autocmd("FileType", {
    group = vim.api.nvim_create_augroup("NvChadTreesitter", { clear = true }),
    callback = function(args)
      ...
      vim.schedule(function()
        ...
        if not opts.auto_install or not supported[language] then
          return
        end
